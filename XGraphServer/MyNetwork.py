...
if __name__ == "__main__":
    p_edges = pd.read_csv('人_边.csv')
    p_nodes = pd.read_csv('人_节点.csv')

    m_edges = pd.read_csv('电影_边.csv')
    m_nodes = pd.read_csv('电影_节点.csv')

    relation = pd.read_csv('关系_边.csv')

    # e = pd.read_csv('测试_人物关系.csv')

    p_nodes = pd.read_csv('laiwu_nodes_station.csv')
    p_edges = pd.read_csv('laiwu_edges_station.csv')
    ...
    net.generate_graph_from_dataframe(v=p_nodes, e=p_edges, r=None, directed=False)
    net.export_network(**p_kw)

    # net.generate_graph_from_dataframe(v=m_nodes, e=m_edges, r=relation, directed=False, overlay=True)
